...
const Login = () => {
  ...
  const loginWithGoogle = async(e) => {
    ...
    // BLOK TRY CATCH
    try {
      // Di sini, sebuah objek GoogleAuthProvider dibuat. 
      // Ini adalah objek yang digunakan untuk mengautentikasi pengguna dengan layanan Google
      const providerGoolge = new GoogleAuthProvider()
      
      // Fungsi signInWithPopup digunakan untuk menampilkan jendela pop-up login dengan provider Google yang telah dibuat 
      // sebelumnya. Hasil dari proses login disimpan dalam variabel result.
      // termasuk informasi tentang pengguna yang berhasil login.
      const result = await signInWithPopup(auth,providerGoolge)
      
      // mengambil kredensial Google dari hasil login. Kredensial ini mengandung informasi seperti token akses yang 
      // akan digunakan untuk mengidentifikasi pengguna.
      const credentialGoogle = GoogleAuthProvider.credentialFromResult(result)
      
      // Di sini, token akses (access token) diekstrak dari kredensial Google. 
      // Token akses ini adalah cara agar pengguna dapat diidentifikasi secara aman setelah login
      const token = credentialGoogle.accessToken

      // ID pengguna diambil dari objek pengguna (user) dalam hasil login. 
      // Ini adalah ID unik yang akan digunakan untuk mengidentifikasi pengguna dalam aplikasi.
      const userId = result.user.uid

      // Objek documenr yang akan disimpan ke dalama collection users
      const docRef = {
      ...
      // Menggunakan setDoc, objek dokumen docRef disimpan di koleksi "users" dengan ID pengguna sebagai kunci di database 
      // atau Firestore.
      await setDoc(doc(db,"users",userId),docRef)

      // ID pengguna yang berhasil login disimpan di penyimpanan lokal (localStorage) dengan kunci 'LOGGED_IN'. 
      // Ini digunakan untuk menandai bahwa pengguna sudah login.
      // Lalu akan diredirect ke halaman dashboard ketika state isLoggedIn bernilai true
      localStorage.setItem('LOGGED_IN',userId)
      setisLoggedIn(true)
    } catch (error) {
